...
- A resident title must identify the utility, project, or other named subject when the source provides one. Preserve that name in the title instead of using a generic heading such as Application for implementation of changes in rates. Its citation must include the name and subject in one contiguous source span.
- Write the title as a short headline for residents about the named subject and action. Preserve the requested official record identifier in sourceRecordId when applicable; leave other ordinance, resolution, account, and contract numbers in the citations. Keep a number in the title or summary only when needed to distinguish the subject or explain the action. Never change an official identifier or the requested record identity.
- Use familiar words when they preserve the source meaning, such as property tax for ad valorem property tax, up to for not to exceed, and seek bids for advertise to receive bids. Preserve qualifications and legal distinctions. Do not invent an acronym expansion, technical definition, project purpose, or effect the source does not establish.
- Write plainLanguageSummary using only what the text states. Do not speculate about effects, motives, or outcomes.
- Write plainLanguageSummary as a short news brief, usually one or two sentences. Lead with the specific action and named subject, then add a supported detail such as the amount, scope, recipient, or report period. Avoid procedural openings such as A motion concerned or The records cover and avoid restating the title without adding information. A short source can warrant a single short sentence.
- Match the explanation to this record. Receiving a required ambulance report establishes receipt of that report; it does not by itself establish a new contract, changed service, or satisfactory performance. A purchase authorization establishes permission to buy, not delivery or an improvement in service. Keep proposal, authorization, and completed action distinct.
...
- For an agenda, explicit discussion, a hearing, a possible vote, or final-adoption consideration supports lifecycleState scheduled. An agenda never proves that the scheduled outcome happened.
- A vote to retain a consultant for a broader pending application is not a final disposition of that application. When the excerpt does not establish the underlying application stage, use lifecycleState unknown and cite only the procedural vote sentence as its context. Do not prepend a separate discussion heading or combine adjacent table rows into that citation.
