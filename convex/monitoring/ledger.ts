...
async function retryFailedTarget(ctx: MutationCtx, target: Doc<'documentInventoryTargets'>, runId?: Id<'pipelineRuns'>) {
  ...
  const budgetPaused = stages.some(stage => stage.errorClass === 'monitoring_daily_limit' || stage.errorClass === 'ai_spending_limit' || stage.errorDetail?.includes('monitoring_daily_limit'))
