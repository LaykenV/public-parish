...
export function inventoryContract(value: InventoryResult, source: string, bodyName: string, priorLocators: string[] = [], allowedSourceKinds?: string[], startsAt?: number): string | null {
  ...
  for (const target of value.targets) {
    // A literal excerpt can cross a Markdown boundary that normalizes
    // differently in the complete source and in the shorter excerpt.
    const cited = source.includes(target.excerpt) || normalized.includes(normalizeForMatch(target.excerpt))
    if (!target.title.trim() || target.title.length > 300 || target.excerpt.length > 240 || !target.excerpt.trim() || !cited) return `Inventory target citation does not resolve for ${JSON.stringify(target.title.slice(0, 120))}. Copy a contiguous source excerpt at most 240 characters, without omissions or ellipses. Rejected excerpt: ${JSON.stringify(target.excerpt.slice(0, 240))}`
