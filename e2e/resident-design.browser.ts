...
for (const slug of ['meta-richland', 'spacex-pecan-island', 'applied-digital-boyce']) {
  test(`${slug} keeps story navigation, citations and Ask reachable`, async ({ page }, testInfo) => {
    ...
    const storyTitle = await article.getByRole('heading', { level: 1 }).innerText()
    ...
    if ((page.viewportSize()?.width ?? 1280) <= 768) {
      ...
      await expect(drawer.getByRole('heading', { level: 1 })).toHaveText(storyTitle)
      await expect(drawer.locator('.ask-screen-context')).toHaveText('Ask Public Parish')
