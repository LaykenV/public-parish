...
function StoryImage({ media, compact = false }: { media: PublicStory['media'], compact?: boolean }) {
...
export function FeaturedStories() {
  ...
  return <section id="stories" className="pp-stories" aria-labelledby="stories-title">
    <header><h1 id="stories-title">Louisiana stories, with the official evidence.</h1><p>Free to read. Ask a question, inspect the sources, and follow what changes.</p><a href="#local-content">Jump to local decisions</a></header>
    {stories === undefined ? <p role="status">Loading reviewed stories...</p> : !stories.length ? <p>Featured stories are under review. Browse published local decisions below.</p> :
      <div className="pp-story-grid">{stories.map(story => <article key={story.id} className={story.rank === 0 ? 'pp-story-card lead' : 'pp-story-card'}>
        ...
        <p className="pp-story-meta">{story.geography.join(' · ')}</p>
        <h2><Link to="/stories/$storySlug" params={{ storySlug: story.slug }}>{story.payload.title.text}</Link></h2>
        <p>{story.payload.summary.text}</p>
        <p className="pp-story-meta">Reviewed through {story.reviewedThrough}{story.mode === 'limited' ? '. Some questions remain unanswered.' : '.'}</p>
      </article>)}</div>}
  </section>
}
...
export function StoryPage({ slug }: { slug: string }) {
  ...
  if (result === undefined) return <main className="pp-page" id="resident-main"><p role="status">Loading story...</p></main>
  if (!result.story) return <main className="pp-page" id="resident-main"><h1>{result.state === 'withdrawn' ? 'Story withdrawn' : 'Story unavailable'}</h1><p>{result.reason}</p><Link to="/">Return to Home</Link></main>
  const story = result.story
  return <main className="pp-page pp-story-detail" id="resident-main">
...
function Statement({ statement, story }: { statement: { text: string, evidenceKeys: string[] }, story: PublicStory }) {
  return <p>{statement.text}{' '}{statement.evidenceKeys.map(key => {
    const index = story.evidence.findIndex(source => source.key === key)
    return index >= 0 ? <a key={key} className="pp-story-citation" href={`#story-source-${index}`} onClick={() => { const target = document.getElementById(`story-source-${index}`); if (target instanceof HTMLDetailsElement) target.open = true }} aria-label={`Inspect source ${index + 1}`}>[{index + 1}]</a> : null
  })}</p>
}
