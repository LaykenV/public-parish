...
an accepted official outcome document in this slice. The August 12 minutes
...
3. Extend the current manifest lineage. Confirm the actual current production
   ...
   outcome, with a change assessment against the exact current draft hash.
4. Approve the exact input, draft and review hashes only after deterministic
   checks pass. Production publication and real email delivery require the
   owner's release authorization. A material change gets one update event;
   an unchanged procedural summary does not.
5. On an authorized release, verify the open story updates, the controlled
   follower receives one sourced email, its management link works, and a reply
   stays within current evidence. Keep recipient details and receipts private.
